#!/usr/bin/env python3
"""Generate the single Torch consumable card (assets/1x/torch.png + 2x), atlas key `bc_torch`.

Same card chrome as the tool cards (gen_tool_cards.py) so the Torch reads as part of the
BalaCraft tool family -- BUT, per the design, with two differences:
  1. NO material plate on top: the rounded nameplate tab is removed and the card top is
     smoothed back to clean grey body (the torch has no material tier).
  2. NO durability bar: the torch is a single-use consumable, so the green bar block is
     removed and smoothed to clean body too.
The torch sprite is the authentic Minecraft texture (assets/reference/tools/torch.png),
pasted at 3x (matching the tools) and centred on its visible pixels. The name "TORCH" is
rendered in the same 6px bold pixel alphabet, in the bottom cream nameplate.
...
# Torch sprite is drawn larger than the tools' 3x: the MC torch texture is a thin ~2px-wide
# stick, so 3x leaves it lost on the card. SCALE is tuned for a card-filling torch.
SCALE = int(os.environ.get("TORCH_SCALE", "4"))
...
# ---------- 4) paste the torch sprite (3x), centred on its visible pixels ----------
sp = Image.open(os.path.join(SPRITEDIR, "torch.png")).convert("RGBA")
sp = sp.resize((sp.width * SCALE, sp.height * SCALE), Image.NEAREST)
bbox = sp.getbbox()                                             # tight box of torch pixels
...
if __name__ == "__main__":
    ...
    card.save(os.path.join(OUT1, "torch.png"))
    card.resize((W * 2, H * 2), Image.NEAREST).save(os.path.join(OUT2, "torch.png"))
    print(f"Wrote assets/1x/torch.png {card.size} (+2x).")
    print("Atlas: SMODS.Atlas { key = 'bc_torch', path = 'torch.png', px = 71, py = 95 }")
    print(f"sprite paste at ({paste_x},{paste_y}); torch bbox {bbox} size {bw}x{bh}")
